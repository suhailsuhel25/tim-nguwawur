...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Kelola akun mahasiswa dan dosen pada sistem.</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" id="search-pengguna"
            ...
            <button type="submit" id="btn-filter-pengguna"
        ...
        <div x-show="confirmId !== null"
            ...
            <div x-show="confirmId !== null"
                ...
                <p class="text-sm text-slate-600 mb-5">
                    Apakah Anda yakin ingin menghapus pengguna <strong x-text="confirmName"></strong>?
                </p>
                <div class="flex gap-3 justify-end">
                    ...
                    <form :action="'/admin/users/' + confirmId" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        ...
                        <th class="text-left px-5 py-3.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Nama</th>
                        <th class="text-left px-5 py-3.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Username</th>
                        <th class="text-left px-5 py-3.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Role</th>
                        <th class="text-left px-5 py-3.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Info Tambahan</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($users as $user)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-5 py-4 text-slate-600 font-mono text-xs">{{ $user->username }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-slate-500 text-xs">
                                @if($user->role === 'student' && $user->student)
                                    <div>{{ $user->student->study_program }}</div>
                                    <div class="text-slate-400">Angkatan {{ $user->student->cohort_year }}</div>
                                @elseif($user->role === 'lecturer' && $user->lecturer)
                                    <div>{{ $user->lecturer->phone_number ?? '-' }}</div>
                                @else
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button @click="confirmId = {{ $user->id }}; confirmName = '{{ addslashes($user->name) }}'"
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-slate-500 font-medium">Belum ada pengguna ditemukan.</p>
                                    <a href="{{ route('admin.users.create') }}" class="text-primary text-sm hover:underline">+ Tambah pengguna baru</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
